# coding=utf-8
"""HF GPT-2 model drop-in replacement with optimization from Megatron-LM and DeepSpeed."""
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        ...
        masked_softmax_fusion = config.fused_scaled_masked_softmax
        if masked_softmax_fusion:
            from mstar.megatron.fused_softmax import FusedScaleMaskSoftmax, AttnMaskType
            if self.scale_attn_weights:
                self.scale_attn_factor = 1. / (float(self.head_dim) ** 0.5)
            else:
                self.scale_attn_factor = 1.0
            fp16, bf16 = config.precision == 16, config.precision == "bf16"
            if not is_cross_attention:
                attn_mask_type = AttnMaskType.causal
            else:
                attn_mask_type = AttnMaskType.padding
            attention_softmax_in_fp32 = True  # Only takes effect for the unfused fallback kernel
            self.scale_mask_softmax = FusedScaleMaskSoftmax(
                fp16, bf16, # Only takes effect for the unfused fallback kernel
                attn_mask_type,
    ...
    def _attn_fused(self, query, key, value, attention_mask=None, head_mask=None):
        # attention_scores
        batch_size, partitions = query.size(0), query.size(1)
        query_length, key_length = query.size(2), key.size(2)
        ...
        key = key.reshape((-1, key.size(-2), key.size(-1))).transpose(-1, -2)
        attn_weights = torch.empty(
            batch_size * partitions,
            query_length,
            key_length,
            dtype=query.dtype,
            device=torch.cuda.current_device()).baddbmm_(
                query, key,
                beta=0.0, alpha=self.scale_attn_factor).view(*output_size)
        ...
        attn_weights = self.scale_mask_softmax(attn_weights, attention_mask)
        attn_weights = self.attn_dropout(attn_weights)
...
class Conv1DGeLU(nn.Module):
    """
    1D-convolutional layer as defined by Radford et al. for OpenAI GPT (and also used in GPT-2).
    Basically works like a linear layer but the weights are transposed.
    This version includes fused GeLU
    Args:
    ...
    def forward(self, x):
        ...
        x = bias_gelu_impl(torch.mm(x.view(-1, x.size(-1)), self.weight), self.bias)
        x = x.view(*size_out)
        return x


class GPT2MLP(nn.Module):
    """MLP for GPT-2 with GeLU as activation"""
    def __init__(self, intermediate_size, config):
        super().__init__()
        embed_dim = config.hidden_size
        assert 'gelu' in config.activation_function, "Only GELU is supported"
        if config.fused_gelu:
            self.c_fc = Conv1DGeLU(intermediate_size, embed_dim)
    ...
    def forward_fused(self, hidden_states):
...
class GPT2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, Conv1D, Conv1DGeLU)):
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            if type(past_key_values[0][0]) is not dict:
                past_length = past_key_values[0][0].size(-2)
            else:
                past_length = past_key_values[0][0]["context_past_key"][0].size(
                    -2
                ) + past_key_values[0][0]["incremental_past_key"][0].size(-2)
