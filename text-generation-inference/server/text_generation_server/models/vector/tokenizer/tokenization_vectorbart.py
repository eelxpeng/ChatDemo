...
class VectorBartTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if fairseq_language_codes is None:
            self.FAIRSEQ_LANGUAGE_CODES = FAIRSEQ_LANGUAGE_CODES
        else:
            self.FAIRSEQ_LANGUAGE_CODES = fairseq_language_codes
        ...
        mask_offset = base_offset + len(self.FAIRSEQ_LANGUAGE_CODES)
        ...
        if use_phrases:
            ...
            try:
                ...
                for phrase in merged_phrases:
                    ...
                    i += 1
                logger.info('Merged Phrases Found: Loading V3 Tokenizer\n')
            except:
                self.phrase_dict = None
                self.phrase_ids_to_tokens = {}
                self.phrase_tokens_to_ids = {}
                logger.info('Loading V2 Tokenizer\n')
        ...
        # Ensures tokenizer pretokenization will never split language codes
        self.unique_no_split_tokens = sorted(set(self.unique_no_split_tokens).union(set(self.FAIRSEQ_LANGUAGE_CODES)))

    @property
    def new_tokens(self) -> List[str]:
        if self._new_tokens is None:
            # Find all new tokens at the end of the vocabulary:
            existing_specials = self.FAIRSEQ_LANGUAGE_CODES + USER_DEFINED_SYMBOLS
            self._new_tokens = [
    ...
    def _convert_token_to_id(self, token: str) -> int:
        ...
        elif token in self.added_tokens_encoder.keys():
    ...
    def _convert_id_to_token(self, index: int) -> str:
        ...
        elif index in self.added_tokens_encoder.values():
            for k, v in self.added_tokens_encoder.items():
                if index == v:
                    return k
    ...
    def set_src_lang_special_tokens(self, src_lang: str) -> None:
        ...
        if src_lang == '<prog_SQL>':
            self.cur_lang_code_id = self.convert_tokens_to_ids('<prog_SQL>')
        else:
