...
from text_generation_server.models.vectorgpt import VectorGPT, VectorGPTSharded
...
def get_model(
    ...
    if model_type == "gpt2":
        logger.info("get_model at rank %s for world size %s" % (os.environ["RANK"], os.environ["WORLD_SIZE"]))
        if sharded:
            return VectorGPTSharded(model_id, revision, quantize=quantize)
        else:
            return VectorGPT(model_id, revision, quantize=quantize)
